...
let currentMatches: MatchedField[] = [];
...
function findDomElement(domField: DomField): HTMLElement | null {
  // Try by id first
  if (domField.id) {
...
function showTooltip(match: MatchedField, circleEl: HTMLDivElement): void {
  dismissTooltip();
...
function renderOverlays(matches: MatchedField[]): void {
  clearOverlays();
...
function repositionCircles(): void {
  if (!shadowRoot) return;
  const circles = shadowRoot.querySelectorAll(".fp-circle");
  circles.forEach((circle, i) => {
    if (i >= currentMatches.length) return;
    const match = currentMatches[i];
    const el = findDomElement(match.domField);
    if (!el) return;
    const rect = el.getBoundingClientRect();
    (circle as HTMLElement).style.left = rect.right + window.scrollX - 12 + "px";
    (circle as HTMLElement).style.top = rect.top + window.scrollY - 12 + "px";
  });
}
...
function autofillAll(): void {
  let filled = 0;
  currentMatches.forEach((match) => {
    if (!match.analysis.suggested_value) return;
    const el = findDomElement(match.domField);
    if (!el) return;
